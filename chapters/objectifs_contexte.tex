...
% Figure principale d'architecture globale
\begin{figure}[H]
	\centering
...
\end{figure}
% COMMENTAIRE:
% Ce schéma doit représenter une vue d'ensemble de l'infrastructure.
% Éléments attendus :
% - Couches horizontales : Infrastructure physique (Proxmox), Infrastructure as Code (Terraform, Ansible), Kubernetes, Observabilité, Sécurité, CI/CD, Services internes.
% - Flèches montrant les interactions.
% - Icônes des principaux outils (Kubernetes logo, Git, Vault, etc.).
% PROMPT IA :
% "Detailed architecture diagram showing a virtualized infrastructure with Proxmox hypervisors, Terraform and Ansible automation layers, Kubernetes orchestration, monitoring stack (Grafana, Prometheus, Loki, Tempo), Vault for secrets management, GitLab for CI/CD, pfSense firewall, and internal services like Harbor and Nextcloud. Use labeled boxes and arrows to illustrate flows."

Cette section détaille les principaux composants et leur articulation.
...
% COMMENTAIRE:
% Schéma en 3 étapes :
% 1. Définition des modules Terraform.
% 2. Execution Terraform Apply -> Création des VMs Proxmox.
% 3. Résultat : Infrastructure provisionnée.
% Icônes : Fichiers HCL, terminal, Proxmox nodes.
% PROMPT IA :
% "Step-by-step diagram showing Terraform provisioning: modules definition in HCL files, Terraform Apply process creating virtual machines in Proxmox cluster, final state with configured VMs. Include labels and arrows."
...
% - Playbooks et rôles.
% - Exécution SSH sur les machines provisionnées.
% - Résultat : VMs configurées (Kubernetes, monitoring, sécurité).
% PROMPT IA :
% "Illustration showing Ansible Control Node running playbooks and roles over SSH to multiple virtual machines, configuring Kubernetes, monitoring tools, and security settings. Use clear arrows and labels."
...
% COMMENTAIRE:
% Schéma montrant :
% - Développeur poussant des manifests dans Git.
% - Argo CD détecte les changements.
% - Synchronisation automatique avec Kubernetes cluster.
% - Flèches indiquant le flux.
% PROMPT IA :
% "GitOps workflow diagram showing a developer committing Kubernetes manifests to a Git repository, Argo CD detecting changes and synchronizing them to the Kubernetes cluster automatically. Include icons and clear arrows."
...
% - Loki (logs)
% - Tempo (traces)
% - Kubernetes Nodes comme sources de données
% PROMPT IA :
% "Architecture diagram of observability stack including Prometheus for metrics, Grafana for visualization, Loki for log aggregation, and Tempo for tracing, all connected to Kubernetes nodes as data sources. Use labeled arrows."
...
% COMMENTAIRE:
% Schéma montrant :
% - Applications demandant des secrets.
% - Vault générant des tokens temporaires.
% - Flux chiffrés.
% PROMPT IA :
% "Secrets management diagram showing applications requesting secrets from Vault, Vault generating short-lived tokens and delivering encrypted credentials, with arrows indicating secure flows."
...
% Schéma avec :
% - pfSense Firewall.
% - Zones réseau (LAN, DMZ, WAN).
% - VPN tunnel pour administrateurs.
% - Règles filtrant les flux.
% PROMPT IA :
% "Network security diagram showing pfSense firewall with LAN, DMZ, and WAN zones, VPN tunnel for administrators, and rules controlling inbound and outbound traffic. Use clear labels."
...
% - Tous hébergés sur Kubernetes.
% PROMPT IA :
% "Diagram showing internal services deployed on Kubernetes: GitLab for CI/CD, Harbor for container registry, YouTrack for issue tracking, Nextcloud for file sharing. Connect all components to Kubernetes nodes."
...
% COMMENTAIRE:
% Schéma illustrant :
% - Développeur poussant du code.
% - Pipeline GitLab CI (stages : build, test, deploy).
% - Déploiement automatique sur Kubernetes.
% PROMPT IA :
% "CI/CD pipeline diagram showing a developer committing code to GitLab, triggering build, test, and deploy stages, with the application deployed to Kubernetes cluster. Use clear stages and arrows."
...
\end{figure}
% COMMENTAIRE:
% Schéma final résumant toutes les couches :
...
% - Services internes
% PROMPT IA :
% "Comprehensive architecture diagram summarizing provisioning (Terraform, Ansible), orchestration with Kubernetes, observability stack, security with Vault and pfSense, CI/CD with GitLab, and internal services. Use layers and clear labels."
